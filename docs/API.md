...
| `POST` | `/api/projects/:id/skills/import` | Import a skill from a GitHub `url`. Accepts a `blob`/`tree`/`raw` link to a `SKILL.md` (or its folder), **or a bare repo URL** `github.com/<owner>/<repo>` (SYM-52): the bare form resolves the repo's default branch and probes the repo root then `skills/` for a `SKILL.md`, so a flat single-skill repo imports without a deep link (a `skills/<name>/` multi-skill repo routes through install instead — the error says so). A folder/tree/bare URL (anything not ending in `.md`) also fetches the skill directory's sibling files into `files` so multi-file skills import completely (SYM-50, bounded by file-count/byte/depth caps); an explicit `*.md` URL stays single-file. Fetch failure / `SKILL.md` not found / rate limit → `502`; duplicate → `409`. → `201` |
